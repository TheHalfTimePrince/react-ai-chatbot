...
const ChatbotRoot = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("fixed bottom-4 right-4 z-50", className)} {...props} />
  )
)
ChatbotRoot.displayName = "ChatbotRoot"

interface ChatbotTriggerProps extends React.ComponentPropsWithoutRef<typeof Button> {
...
const ChatbotWindow = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
...
const ChatbotHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { onClose?: () => void }>(
  ({ className, onClose, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex justify-between items-center p-4 border-b border-border cursor-pointer", className)}
      onClick={onClose}
      {...props}
    />
  )
...
const ChatbotHeaderTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
...
const ChatbotHeaderAvatar = React.forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<typeof Avatar>>(
  (props, ref) => <Avatar ref={ref} {...props} />
)
ChatbotHeaderAvatar.displayName = "ChatbotHeaderAvatar"

const ChatbotCloseButton = React.forwardRef<HTMLButtonElement, React.ComponentPropsWithoutRef<typeof Button>>(
  (props, ref) => (
    <Button ref={ref} variant="ghost" size="icon" className="text-foreground" {...props}>
      <X className="w-4 h-4" />
    </Button>
  )
)
...
const ChatbotNotificationButton = React.forwardRef<HTMLButtonElement, React.ComponentPropsWithoutRef<typeof Button> & { notificationPermission: NotificationPermission }>(
  ({ notificationPermission, className, onClick, ...props }, ref) => (
    <Button
      ref={ref}
      variant={notificationPermission === 'granted' ? 'secondary' : 'ghost'}
      size="icon"
      className={cn("text-foreground", notificationPermission === 'granted' ? 'bg-secondary' : '', className)}
      title={notificationPermission === 'granted' ? 'Disable notifications' : 'Enable notifications'}
      onClick={onClick}
      {...props}
    >
...
const ChatbotMessageList = React.forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<typeof ScrollArea>>(
  (props, ref) => <ScrollArea ref={ref} className="flex-grow p-4" {...props} />
)
ChatbotMessageList.displayName = "ChatbotMessageList"

const ChatbotMessage = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { role: 'user' | 'assistant' }>(
...
const ChatbotInputForm = React.forwardRef<HTMLFormElement, React.FormHTMLAttributes<HTMLFormElement>>(
  ({ className, ...props }, ref) => (
    <form ref={ref} className={cn("p-4 border-t border-border", className)} {...props} />
  )
)
ChatbotInputForm.displayName = "ChatbotInputForm"

const ChatbotInput = React.forwardRef<HTMLInputElement, React.ComponentPropsWithoutRef<typeof Input>>(
  ({ className, ...props }, ref) => (
    <Input
      ref={ref}
...
const ChatbotSubmitButton = React.forwardRef<HTMLButtonElement, React.ComponentPropsWithoutRef<typeof Button>>(
  (props, ref) => (
...
const ChatbotThinkingIndicator = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("flex items-center space-x-2 text-muted-foreground", className)} {...props}>
...
export const createNotificationSound = () => {
    ...
    for (let channel = 0; channel < 2; channel++) {
      ...
      for (let i = 0; i < impulseLength; i++) {
        impulseData[i] = (Math.random() * 2 - 1) * Math.exp(-i / (sampleRate * 0.1))
      }
    }
    ...
    const playTone = (frequency: number, startTime: number) => {
      ...
      gainNode.gain.linearRampToValueAtTime(0.2 * sustain, startTime + attack + decay)
    ...
    playTone(1046.50, context.currentTime + 0.2) // C6 note (perfect 5th above F5)
  } 
...
export function Chatbot({
    ...
    React.useEffect(() => {
      ...
      // Only trigger notification if we have a new AI message
      if (aiMessages > previousAiMessageCount.current) {
        createNotificationSound()
        if ('Notification' in window && notificationPermission === 'granted') {
          new Notification(`New message from ${title}`, { body: 'You have a new reply!' })
        }
      }
    ...
    return (
      <ChatbotRoot>
        {!isOpen && <ChatbotTrigger onClick={toggleChatbot} attentionText={attentionText} />}
        {isOpen && (
          <ChatbotWindow>
